...
class selfPlayer:
    ...
    def drawHelper(self, app):
        drawCircle(self.x, self.y, 30, fill='yellow')
        numFlowers = 0
        colors = []
        for flower in app.player.pollenInventory:
            if flower.color not in colors:
                drawCircle(self.x+ numFlowers*20, self.y + 50, 6, fill=flower.color)
                colors.append(flower.color)
                numFlowers += 1

    def helperOnStep(self):
        self.x += self.velocityX
        self.y += self.velocityY
    ...
    def target(self, app):
        # target is not none
        if (self.target in app.flowerList):
            if (self.target.type == "pollinator") and (self.gathered == False):
                self.velocityX = (self.target.x - self.x)*self.constant
                self.velocityY = (self.target.y - self.y)*self.constant
            elif (self.target.type == "notAPollinator") and (self.pollinated > 0):
                self.velocityX = (self.target.x - self.x)*self.constant
                self.velocityY = (self.target.y - self.y)*self.constant
            else:
                self.target = None
...
def onMouseMove(app, mouseX, mouseY):
    ...
    if (app.helper.target == None):
        distances = dict()
        for flower in app.flowerList:
            if (flower.type == "pollinator") and (flower.gathered == False):
                distances[flower] = distance(app.helper.x, app.helper.y, flower.x,flower.y)
            elif (flower.type == "notAPollinator") and (flower.pollinated > 0):
                distances[flower] = distance(app.helper.x, app.helper.y, flower.x,flower.y)
        app.helper.target = min(distances, key = distances.get)
    app.helper.target(app)

def onStep(app):
    # step for the player
    app.player.removePollen()
    app.player.playerOnStep()
    app.helper.helperOnStep()
